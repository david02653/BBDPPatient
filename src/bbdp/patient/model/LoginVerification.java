...
public class LoginVerification {
	...
	public HashMap verification(DataSource datasource, String account, String password, String uuid) {
		String result = "";
		String s, p, patientID = null;
		Connection con = null;
		try {
			con = datasource.getConnection();
			Statement st = con.createStatement();
		    ResultSet rs = st.executeQuery("select patientID, account, password from patient");
		    while (rs.next()) {
	...
	// 判斷登入	
	// 此用戶已在其他裝置登入
	public HashMap judgeLogin(DataSource datasource, String patientID, String uuid) {
		Connection con = null;
		...
		try {
			con = datasource.getConnection();
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery("select uuid from patient where patientID = " + patientID);
	...
	// 更新uuid
	public boolean updateUUID(DataSource datasource, String patientID, String uuid) {
		Connection con = null;
		Gson gson = new Gson();
		try {
			con = datasource.getConnection();
		    ...
		    int update = st.executeUpdate("update patient set uuid = '"+uuid+"' where patientID = '"+patientID+"' ");
		    st.close();//關閉st
